...
### LCD Display Setup – Choose Your Method

#### ⚠️ Method 1: Full Desktop Display (Raspberry Pi OS ONLY)
**Use this if:**
- ✅ Running **Raspberry Pi OS** (not Ubuntu/other distros)
- ✅ Want the LCD as a second desktop screen
- ⚠️ **Warning:** This method modifies boot config and may break boot sequence on non-RPi-OS systems
...
cd LCD-show/
sudo ./LCD35-show 180 # orientation based on screen preference 
```

---

#### ✅ Method 2: Dedicated Clock Display (ANY Linux Distro)
**Use this if:**
- ✅ Running **Ubuntu, Debian, or any Linux distro** (not just RPi OS)
- ✅ Want a reliable 24/7 clock display on the LCD
- ✅ Want HDMI as your main monitor without boot issues
- ✅ Don't want LCD-show breaking your boot sequence

**What you get:**
- 🕐 Big white digital clock on black background
- 📅 Date display below the time
- 🖥️ HDMI works normally as main display
- 🔄 Auto-starts on boot
- 🛡️ No boot sequence conflicts

**Quick Setup:**
```bash
# 1. Clone this repo (or download lcd_clock folder)
git clone https://github.com/lilking2007/Rassberrypi5-cyber-infra.git
cd Rassberrypi5-cyber-infra/lcd_clock
...
**After reboot:**
- Your HDMI monitor = Normal desktop
- Your LCD screen = 24/7 clock display

**Customize the clock:**
Edit `/usr/local/bin/lcd_clock.py` to change colors, time format, or hide the date.
See `lcd_clock/LCD_CLOCK_GUIDE.md` for full customization options.

---

**Base OS:** Raspberry Pi OS 64-bit (Lite)  
**Core Stack:** Docker, Portainer, Pi-hole, ntopng, n8n, Filebrowser, OpenClaw
---
...
```
**Password reset/set** `docker exec -it pihole pihole setpassword`
...
**Restart Policy:** Always  
## step 5.2.2 - Deployment via portainer using stacks 
Delete the broken container: `docker rm -f filebrowser`
```yaml
version: '3'
services:
  filebrowser:
...
|--------|--------------|--------------------|
| LCD Clock | Displays on LCD screen | No credentials (auto-start) |
...
| Problem | Solution |
|--------|----------|
| LCD-show breaks boot on Ubuntu | Use dedicated LCD Clock method instead (works on any distro) |
...
###############################################################################
# 4. LCD CLOCK SETUP (OPTIONAL - WORKS ON ANY LINUX DISTRO)
###############################################################################
cd lcd_clock
chmod +x setup_lcd_clock.sh
./setup_lcd_clock.sh
...
echo "LCD CLOCK: Running on LCD screen (if setup was run)"
...
🕐 LCD Clock display option for any Linux distro.
---
